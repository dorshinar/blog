...
At work, we started working on a component library distributed through NPM and consumed by several applications. We bundled our library with [tsup](https://tsup.egoist.dev/) which worked great.
...
Importing our `<Button />{:jsx}` component, [Import Cost](https://marketplace.visualstudio.com/items?itemName=wix.vscode-import-cost) reported 97Kb added to our bundle. Don't get me wrong, we have nice buttons, but not 97Kb worth. Importing `*.svg` icons resulted in a similar bundle size added to our application's bundle.
...
As it turns out, creating a tree-shakable library can be tricky. In this post I'll walk you through how I set our component library to be tree-shakable with `tsup`.
...
But for the `Dialog` component we'll use the excellent `@radix-ui/react-dialog`. I pulled the example from their [docs](https://www.radix-ui.com/primitives/docs/components/dialog).
...
To consume our components in an application we bundle them together, produce an `index.js` file, publish to `npm` and we have a library.

Different applications may import different components from our library. Some only need the button, others the dialog and some need both. Ideally, when we are building the consuming application we would like the bundle of each application to only contain the code it _actually_ uses.

Without tree-shaking, each application that uses the `<Button>{:jsx}` component will have 51.9kB _it doesn't need_. As our library grows larger, this will become an issue. It is not unheard of to have component libraries that contain 100kB+, where each application only needs a sliver of.
...
To get tree-shaking working reliably, distribute your code in ESM format. ESM uses statically analyzable `import/export` statements which bundlers can use when bundling our code.

Webpack and rollup support it as well as [modern browsers](https://caniuse.com/?search=esm%20module), so it's a safe compilation target.
...
If you are bundling your library with typescript, set the `compilerOptions.module` to `ES2015/ES6/ES2020/ES2022/ESNext`.
...
This is critical because while side effects are rare, if you do have code that is not pure, like polyfills or otherwise, you might not want it excluded from your bundle when the imported module is not referenced. This can include CSS files imported but not referenced.
...
For the most part, bundlers can exclude the entire file from your build. Rollup and webpack may tree-shake unused functions and variables written in the same file. This behavior is not very predictable so it's best to stick to the lowest common denominator.
...
To solve that you can bundle your library so that you still have a single entry point, but it points to separate files that can be excluded individually. Notice that in reality your `dist` folder will probably be messier than that, but the idea is the same.
...
The `exports` field tells bundlers where to import files from. The `"."` identifier means that imports from the root of the package (`import * from "package-name"`) will be imported from the `default` field.
...
The `import` field is for ESM bundlers, and the `require` field is for CJS bundlers. The `default` field is for the default import, and the `types` field is for telling typescript where to find the types for our files.
...
To do that, I've created a [repository](https://github.com/dorshinar/treeshaking-demo) with an example library and 2 applications using different bundlers - a `Next.js` application and a `Vite` application.
