...
Now that we understand _why_ the rules exist, let's get practical. A "good effect" in the concurrent world is one that can handle being called multiple times without breaking your app.
...
function ExternalSubscription() {
  ...
  useEffect(() => {
    const subscription = someExternalLibrary.subscribe(setData);
...
```tsx
// Data fetching (use a library instead)
...
// Transforming data (do this during render)
...
The key difference? Good effects are about syncing with the outside world. Bad effects are about managing state or data that React could handle better through other means.
