...
- The UI stays responsive during expensive operations
  ...
  <Message variant="info">
    There are a few more features that benefit from following those rules, such
    as the [Activity API](https://react.dev/reference/react/Activity) and the
    [React Compiler](https://react.dev/learn/react-compiler). I won't cover them
