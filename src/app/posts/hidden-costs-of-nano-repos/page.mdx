...
Now, instead of 1 service talking to a DB we potentially have 4, so it really doesn't make sense to put the `docker-compose.yml` file in any of the repos. So we need _another_ repo to hold the local environment. This repo will have to be cloned as well, and the `README` file will instruct us to clone it.
...
Well, yeah, but not really. Onboarding can have a big impact on the first impression new employees have of the company. Remember - this is the first thing they'll do as new employees. Second, this means moving between projects is harder.

So it's really a ripple effect that starts with onboarding, but doesn't end there.
...
All of a sudden you can't `Cmd+Shift+F` to find something in the codebase. You need to remember which repo it's in, and then search for it in that repo. This is a lot of overhead that is easy to overlook.
...
Either way, the documentation lives elsewhere than the code. This means that it's harder to find things because you wouldn't even know where to look. Do I look in the repo's README? Do I search the company-wide Confluence?
