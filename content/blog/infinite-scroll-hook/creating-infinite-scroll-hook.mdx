...
Sadly, we're usually constrained by computing, network, or both, and that forces us to compromise. In our case, we must give up trying to load the entire list on the client's device. think about it - _very_ large lists full of information can be several megabytes large!
...
The [Intersection Observer API](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API) is the (not so) new kid on the block. It provides an asynchronous API to detect the intersection of elements without viewport.
...
To create an Intersection Observer, you first need to get a ref of the element we wish to be notified of when it enters the viewport.
...
For example, for a threshold of [0.33, 0.67, 1], the callback would fire 3 times - the first when 33% of the element is visible, the second when 50% of the element is visible, and the last when the entire element is visible.
