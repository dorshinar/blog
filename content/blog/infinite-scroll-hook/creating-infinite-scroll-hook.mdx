...
Each `IntersectionObserverEntry` contains some very interesting arguments. I'll leave it up to you to dig through the docs to learn about all of them, but for out purposes we only need one argument - `isIntersecting`. We will touch another cool one at the end.
...
Our hook is complete, but for sake of being thorough I wanted to go over more options of the IntersectionObserver API. Some of you may find them useful.
...
<CodeShiftingBox root />

The third parameter is `threashold`. It accepts either a single number or an array of numbers, which represent the how much of the element should be visible in order to trigger the callback. 

A threashold of 1 would trigger the callback when every pixel of the element is visible. 

<CodeShiftingBox root threshold={1} />

A threshold of 0.5 would trigger the callback when the half of the element is visible.  

<CodeShiftingBox root threshold={0.5} />

If you specify an array, the callback would fire whenever the element's visibility surpasses one of the entries. 

For example, for a threashold of [0.33, 0.67, 1], the callback would fire 3 times - the first when 33% of the element is visible, the second when 50% of the element is visible, and the last when the entire element is visible. 
...
Aside from the `isIntersecting` parameter, we also get `intersectionRatio`, which tells us by how much our element is intersecting with the relevant viewport. This way we can tell whether it's 33% visible, 50% visible or 100% visible.

## Final Words

The Intersection Observer is and awesome API that opens the doors for a lot of fun interactions. 
